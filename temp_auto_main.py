...
class AutoDriver(Node):
    def __init__(self):
        ...
        # Open serial port (optional ΓÇö can be disabled if not needed)
        try:
        ...
        # Subscribe to raw LiDAR scan (for future use ΓÇö optional)
    ...
    def update_combined_obstacle_state(self):
        ...
        if new_obstacle_state != self.obstacle_active:
            ...
            if new_obstacle_state:
                self.get_logger().warn('≡ƒ¢æ Obstacle detected!')
            else:
                self.get_logger().info('Γ£à Obstacle cleared.')
